...
def build_all_stock_datas(tuple_list = None):
    ...
    with concurrent.futures.ThreadPoolExecutor() as executor:
        # 提交所有任务
        future_to_task = {
            executor.submit(build_stock_datas_extend, stock_code, datekey): (datekey, stock_code)
...
def build_stock_datas_extend(stock_code, datekey):
    ...
    if not next_trade_days:
        return None
    res = []
    for next_trade_day in next_trade_days:
        dres = build_stock_datas(stock_code, next_trade_day)
        if not dres:
            continue
        res.append(dres)
    if len(res) != y:
...
def build_evaluater_1to2_data_list(result_tuples):
    ...
    # 严格模式（可选）
    results.sort(key=lambda x: datetime.strptime(x[0]['datekey'], '%Y-%m-%d'))
    res = []
    i = 1
    for cur_results in results:
        ...
        for i in range(y):
            ...
            cur_res_datas = {}
            if i == 0:
                cur_res['datekey'] = result['datekey']
                order_price = result['base_price']
                cur_res['order_price'] = order_price
                cur_res['trade_price'] = order_price
                cur_res['origin_trade_price'] = order_price
                cur_res['stock_code'] = result['stock_code']
            pre_close = result['close']
            limit_down_price, limit_up_price = constants.get_limit_price(pre_close)
            n_close = result['n_close']
            if abs(n_close - limit_up_price) < 0.01:
                cur_res_datas['limit_up'] = 1
            else:
                cur_res_datas['limit_up'] = 0
            if abs(n_close - limit_down_price) < 0.01:
                cur_res_datas['limit_down'] = 1
            else:
                cur_res_datas['limit_down'] = 0
            cur_res_datas['open'] = result['n_open']
            cur_res_datas['close'] = result['n_close']
            cur_res_datas['pre_volumes'] = result['pre_avg_volumes']
            cur_res_datas['n_pre_volume'] = result['n_pre_volume']
...
if __name__ == "__main__":
    res = build_evaluater_1to2_data_list_from_file(1)
    def serialize(obj):
        # 处理列表截断（只保留前100项）
